...
{
	__android_log_print( ANDROID_LOG_FATAL,"Surace","Create:%d\n",(int)pthread_self());
	AndroidApp::init(env,obj);
	...
	if(!g_main)
	{
		...
		__android_log_print( ANDROID_LOG_FATAL,"Surace","1");
		g_main->init(NULL);
		__android_log_print( ANDROID_LOG_FATAL,"Surace","4");
	}
	else
	{
		GLDevice::restore();
...
extern "C" void FUNC(onNdkSurfaceRelease)(JNIEnv* env, jobject obj)
{
	...
	__android_log_print( ANDROID_LOG_FATAL,"Surace","Release:%d\n",(int)pthread_self());
}

extern "C" void FUNC(onNdkSurfaceChanged)(JNIEnv* env, jobject obj, jint width, jint height)
{
	__android_log_print( ANDROID_LOG_FATAL,"Surace","Change:%d\n",(int)pthread_self());
	AndroidApp::init(env,obj);
	if(g_main)
...
//	AndroidApp::init(env,obj);
	...
	if(size >= 5)
		__android_log_print( ANDROID_LOG_FATAL,"Touch","%f,%f\n",data[1],data[2]);
